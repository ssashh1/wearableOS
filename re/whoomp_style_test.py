"""Clean whoomp-style connection: subscribe ONLY to custom chars (no standard 0x2A37).
Tests whether the standard-HR subscription was suppressing the custom command/data channels.
"""
import asyncio
import sys
import time
...
t0 = time.time()
counts = {"cmd_from": 0, "data": 0, "events": 0}
...
def mk(name):
    def cb(_, data):
        ...
        try:
            ...
            if name in ("cmd_from", "data") or counts[name] <= 6:
                print(f"[{time.time()-t0:5.1f}s] {name}: {pkt}", flush=True)
        except Exception as e:
            print(f"[{time.time()-t0:5.1f}s] {name} raw={bytes(data).hex()[:50]} ({e})", flush=True)
    return cb
...
async def main():
    ...
    async with BleakClient(dev) as client:
        print(f"Connected: {client.is_connected} (NO standard-HR subscription)\n", flush=True)
        ...
        await client.start_notify(MEMFAULT, mk("memfault") if "memfault" in counts else mk("data"))
        ...
        await send(client, CommandNumber.GET_BATTERY_LEVEL); await asyncio.sleep(1)
        await send(client, CommandNumber.GET_CLOCK); await asyncio.sleep(1)

        print("\n--- realtime HR on for 5s ---", flush=True)
